- [0 元语言](foundation)
  - [0.0 导论](foundation.essential)
  - [0.1 离散性](foundation.discrete)
    - [0.1.0 定义](foundation.discrete.base)
    - [0.1.1 实例](foundation.discrete.instance)
    - [0.1.2 列表](foundation.discrete.list)
  - [0.2 可枚举性](foundation.enumeration)
    - [0.2.1 可选值序列视角](foundation.enumeration.maybeview)
    - [0.2.2 累积列表视角](foundation.enumeration.listview)
      - [0.2.2.0 定义](foundation.enumeration.listview.base)
      - [0.2.2.1 实例](foundation.enumeration.listview.instance)
    - [0.2.3 普通视角](foundation.enumeration.plainview)
  - [0.3 构造主义反推基础](foundation.reverse)
  - [0.9 符号表](foundation.cheatsheet)
- [1 一阶逻辑](fol)
  - [1.0 语言](fol.language)
    - [1.0.1 语言的定义](fol.language.base)
    - [1.0.2ᐞ 语言的扩张](fol.language.extension)
  - [1.1 语法](fol.syntax)
    - [1.1.0 语法蕴含](fol.syntax.base)
    - [1.1.1 新变元与闭公式](fol.syntax.fresh)
    - [1.1.2 公式的离散性](fol.syntax.discrete)
    - [1.1.3 公式的枚举](fol.syntax.enumeration)
    - [1.1.4ᐨᐞ 证明的枚举](fol.syntax.proof-enumeration)
    - [1.1.5ᐨ 替换的性质](fol.syntax.substitution)
    - [1.1.6 可容许规则](fol.syntax.admissible)
    - [1.1.7ᐨ 理论规则](fol.syntax.theory)
    - [1.1.8ᐞ 语法稳定性](fol.syntax.stability)
  - [1.2 语义](fol.semantics)
    - [1.2.0 语义蕴含](fol.semantics.base)
    - [1.2.1ᐨ 取值的性质](fol.semantics.evaluation)
  - [1.3 可靠性](fol.soundness)
  - [1.4 理论的扩张](fol.extension)
  - [1.5ᐞ 完备性](fol.completeness)