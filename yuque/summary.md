- [0 元语言](foundation)
  - [0.0✩ 导论](foundation.essential)
  - [0.1ᐨᐞ 可枚举性](foundation.enumeration)
    - [0.1.1 可选值序列视角](foundation.enumeration.maybeview)
    - [0.1.2 累积列表视角](foundation.enumeration.listview)
      - [0.1.2.0 定义](foundation.enumeration.listview.base)
      - [0.1.2.1 实例](foundation.enumeration.listview.instance)
    - [0.1.3 普通视角](foundation.enumeration.plainview)
  - [0.9 符号表](foundation.cheatsheet)
- [1 一阶逻辑](fol)
  - [1.0 语言](fol.language)
  - [1.1 语法](fol.syntax)
    - [1.1.0 语法蕴含](fol.syntax.base)
    - [1.1.1ᐞ 公式的离散性](fol.syntax.discreteness)
    - [1.1.2ᐞ 公式的枚举](fol.syntax.enumeration)
    - [1.1.3ᐞ 新变元与闭公式](fol.syntax.fresh)
  - [1.2 语义](fol.semantics)
    - [1.2.0 语义蕴含](fol.semantics.base)
    - [1.2.1ᐨ 公式取值的性质](fol.semantics.properties)
  - [1.3✩ 可靠性](fol.soundness)
  - [1.4ᐞ 亨金构造](fol.henkin)
  - [1.5✩ᐞ 完备性](fol.completeness)