...
∷ₛ⊨ₜ↑ₜ x 𝓋 = term-elim (λ _ → refl) H where
  H : ∀ f t⃗ → (∀ t → t ∈⃗ t⃗ → 𝓋 ⊨ₜ t ≡ (x ∷ₛ 𝓋) ⊨ₜ ↑ₜ t) →
    𝓋 ⊨ₜ (f $̇ t⃗) ≡ (x ∷ₛ 𝓋) ⊨ₜ ↑ₜ (f $̇ t⃗)
  H f t⃗ IH = cong (funMap f) H2 where
    H2 : 𝓋 ⊨ₜ⃗ t⃗ ≡ (x ∷ₛ 𝓋) ⊨ₜ⃗ ↑ₜ⃗ t⃗
    H2 rewrite ⊨ₜ⃗≡map⃗ 𝓋 t⃗ | ⊨ₜ⃗≡map⃗ (x ∷ₛ 𝓋) (↑ₜ⃗ t⃗) | []ₜ⃗≡map⃗ t⃗ (#_ ∘ suc) = H3 where
      H3 = map⃗ (𝓋 ⊨ₜ_) t⃗                              ≡⟨ map-ext IH ⟩
           map⃗ ((x ∷ₛ 𝓋) ⊨ₜ_ ∘ ↑ₜ) t⃗                  ≡⟨ map-∘ _ _ _ ⟩
           map⃗ ((x ∷ₛ 𝓋) ⊨ₜ_) (map⃗ _[ #_ ∘ suc ]ₜ t⃗)  ∎
...
⊨ᵩ-∘ 𝓋 (∀̇ φ) σ = ↔-cong-Π λ x →
  (x ∷ₛ (𝓋 ⊨ₜ_) ∘ σ) ⊨ᵩ φ               ↔⟨ ↔-cong (_⊨ᵩ φ) (funExt $ H x) ⟩
