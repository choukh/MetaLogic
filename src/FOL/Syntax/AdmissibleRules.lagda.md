...
语境弱化规则允许我们通过在弱化的语境中证明某公式, 来说明原语境中就能证明该公式.
...
**<u>规则</u>** 如果对 `φ` 做 `n` 次全称量化再做 `n` 次提升得到的公式可证, 那么 `φ` 可证.
...
**<u>规则</u>** 如果 `φ` 的最大全称量化式可证, 那么 `φ` 可证.
...
**<u>定义</u>** 语境内化式: 设语境 `Γ` 等于 `φ₁ ∷ φ₂ ∷ ... ∷ [ φₙ ]`, 公式 `φ₁ →̇ φ₂ →̇ ... →̇ φₙ →̇ φ` 叫做 `Γ ⊢ φ` 的语境内化式, 记作 `Γ ⇢ φ`, 其中 `_⇢_` 也叫大蕴含符号.
...
**<u>规则</u>** 语境内化: 如果 `Γ ⇢ φ` 的最大全称量化式在空语境下可证, 那么 `φ` 在 `Γ` 下可证.
