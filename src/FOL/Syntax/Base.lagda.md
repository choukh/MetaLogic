...
**<u>定义</u>** 理论 `𝒯` 下对公式 `φ` 的证明树, 记作 `𝒯 ⊢ᵀ φ` (“不能构造 `𝒯 ⊢ᵀ φ`” 记作 `𝒯 ⊬ᵀ φ`), 是一个二元组
...
```agda
_⊢ᵀ_ _⊬ᵀ_ : Theory → Formula → 𝕋
𝒯 ⊢ᵀ φ = Σ̅ λ Γ → Γ ᴸ⊆ᴾ 𝒯 ∧ Γ ⊢ φ
𝒯 ⊬ᵀ φ = ¬ (𝒯 ⊢ᵀ φ)
```

**<u>定义</u>** 我们说 `φ` 是恒真式 (或者说重言式, tautology), 当且仅当对任意 `Γ` 都有 `Γ ⊢ φ`.
