...
```agda
soundness⟨ 𝒞 ⟩ exp (ImpI H) c 𝓋 𝓋⊨Γ 𝓋⊨φ = soundness⟨ 𝒞 ⟩ exp H c 𝓋 (𝓋⊨φ ⊨∷ 𝓋⊨Γ)
```
...
```agda
soundness⟨ 𝒞 ⟩ exp (ImpE H₁ H₂) c 𝓋 𝓋⊨Γ = soundness⟨ 𝒞 ⟩ exp H₁ c 𝓋 𝓋⊨Γ $ soundness⟨ 𝒞 ⟩ exp H₂ c 𝓋 𝓋⊨Γ
```
...
```agda
soundness⟨ 𝒞 ⟩ exp (AllI H) c 𝓋 𝓋⊨Γ x = soundness⟨ 𝒞 ⟩ exp H c (x ∷ₙ 𝓋) $
...
```agda
soundness⟨ 𝒞 ⟩ exp (AllE {φ} {t} H) c 𝓋 𝓋⊨Γ = φ[t]-intro 𝓋 φ t (soundness⟨ 𝒞 ⟩ exp H c 𝓋 𝓋⊨Γ (𝓋 ⊨ₜ t))
```
...
```agda
soundness⟨ 𝒞 ⟩ exp (FalseE {φ} Γ⊢⊥̇) c 𝓋 𝓋⊨Γ = semanticExplosion (exp c .snd) 𝓋 φ $ soundness⟨ 𝒞 ⟩ exp Γ⊢⊥̇ c 𝓋 𝓋⊨Γ
```
...
```agda
soundness⟨ 𝒞 ⟩ exp (Peirce φ ψ) c 𝓋 _ = exp c .fst 𝓋 φ ψ
```
