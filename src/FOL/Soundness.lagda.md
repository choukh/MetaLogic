...
- 使用语境规则 `Ctx` 构造:
  ...
  - 子证明: `𝓋 ⊨ₛ Γ` 是说 `φ ∈̂ Γ → 𝓋 ⊨ᵩ φ`. 而我们有前提 `φ ∈̂ Γ`, 所以目标蕴含式成立. ▫
...
- 使用全称的引入规则 `AllI` 构造:
  ...
  - 子证明: 由归纳假设, 只需证 `𝓋 ⊨ₛ Γ → x ∷ₙ 𝓋 ⊨ₛ ⇞ Γ`. 由引理 `map⊆P`, 只需证对任意 `φ ∈̂ Γ` 有 `x ∷ₙ 𝓋 ⊨ᵩ ↑ x₁`. 由引理 `∷ₙ⊨ᵩ↑`, 只需证 `𝓋 ⊨ᵩ φ`. 由前提 `𝓋 ⊨ₛ Γ` 和 `φ ∈̂ Γ` 得证. ▫
