...
- 我们说语境 `Γ` 在 `𝓋` 下有效, 记作 `𝓋 ⊨ₛ Γ`, 当且仅当 `𝓋 ⊨ᵩ φ` 对任意 `φ ∈͆ Γ` 成立.
...
```agda
⊨↔⊫ : Γ ⊨⟨ 𝒞 ⟩ φ ↔ set Γ ⊫⟨ 𝒞 ⟩ φ
⊨↔⊫ .⇒ H c 𝓋 𝓋⊫ₛΓ = H c 𝓋 λ φ φ∈Γ → 𝓋⊫ₛΓ φ ∣ φ∈Γ ∣₁
⊨↔⊫ .⇐ H c 𝓋 𝓋⊨ₛΓ = H c 𝓋 λ φ φ∈Γ → 𝟙.rec (isProp⊨ᵩ 𝓋 φ) (𝓋⊨ₛΓ φ) φ∈Γ
```

**<u>引理</u>** 语义稳定性: 标准 `𝒞`-语义蕴含是稳定的.  
**<u>证明</u>** 我们工作在经典-标准⊥语义下, 有前提 `¬ ¬ (𝒯 ⊫⟨ 𝒞 ⟩ φ)`, 且 `𝒯 ⊫ₛ 𝓋`, 要证 `𝓋 ⊨ᵩ φ`. 由经典语义的特性, 只要证 `¬ 𝓋 ⊨ᵩ φ →̇ ⊥̇`. 假设 `𝓋 ⊨ᵩ φ →̇ ⊥̇`, 我们证 `¬ (𝒯 ⊫⟨ 𝒞 ⟩ φ)`. 假设 `𝒯 ⊫⟨ 𝒞 ⟩ φ`, 有 `𝓋 ⊨ᵩ φ`, 与 `𝓋 ⊨ᵩ φ →̇ ⊥̇` 矛盾 (因为由标准⊥语义, `𝓋 ⊨ᵩ ⊥̇` 意味着 `⊥`). ∎
