...
本篇引入一阶逻辑的**项 (term)**, **公式 (formula)**和**证明 (proof)**, 它们共同构成了一阶逻辑的语法部分. 项由变量和函数符号构成; 公式则由关系符号和逻辑符号构成. 粗略类比, 如果说符号相当于字, 那么项和公式则相当于词和句. 注意本篇所有内容都是参数化到语言的.
...
我们将使用 [De Brujin编码](https://en.wikipedia.org/wiki/De_Bruijn_index) 实现量词的绑定, 因此项中使用自然数作为变量(名).
...
项的替换是指把一个项中所使用的所有变量分别替换成一些别的项. 这一操作是 De Brujin编码方案的一部分, 目的是为了实现量词绑定, 具体用法会在后面逐步说明. 首先为了实现项的替换, 我们需要一套替换规则, 它记录了全体自然数 (变量) 与项的一套对应关系.

**<u>定义</u>** 变量替换表 (`σ : Subst`) 是一个将变量映射到项的函数 `ℕ → Term`.
...
**<u>定义</u>** 项 `t` 的提升 `↑ₜ t` 是指将 `t` 中涉及的所有变量 (自然数) 替换为其后继.
...
**<u>定义</u>** 替换表 `σ` 的提升 `↑ₛ σ` 是指对替换表 `σ` 的值域中的项全部做一次提升, 得到替换表 `↑ₜ ∘ σ`, 然后使用希尔伯特旅馆操作把变量 `# 0` 插入到 `↑ₜ ∘ σ` 的头部, 得到 `# 0 ∷ₙ ↑ₜ ∘ σ`.
...
之所以要做此种看似复杂的操作, 是因为后面我们会如此定义量词, 使得量词作用下的 `# 0` 被解释为量词的绑定变量, 它不应该受替换操作影响. 可以看到, `↑ₛ σ` 确实不会替换 `# 0`, 或者说替换后仍是 `# 0`. 而对于非绑定变量 (`# 1`, `# 2`, ...) 的替换, 需要保证替换后不会变成 `# 0`, 因此需要对 `σ` 的值域中的项全部做一次提升, 得到 `↑ₜ ∘ σ`, 以把 `# 0` 空出来. 总而言之, `↑ₛ σ` 对 `σ` 的定义域和值域都整体做了一次提升, 而空出来的 0 的位置仍然是 `# 0`.
...
我们采用精简形式的公式定义, 只采用恒假 `⊥̇`, 蕴含 `→̇` 和全称量化 `∀̇` 这三个符号, 以方便证明一阶逻辑的元性质. 实际使用时, 否定, 合取, 析取, 逻辑等价和存在量化都可以用这三个符号来定义, 而等词则需要作为二元关系符号加入到语言中.
...
**<u>递归定义</u>** 公式的替换 `φ [ σ ]ᵩ` 是指把变量替换表 `σ` 作用到公式 `φ` 中的所有项上.

- 如果 `φ` = `⊥̇`, 则没有项可被替换, 所以仍为 `⊥̇`
- 如果 `φ` = `φ₁ →̇ φ₂`, 则分别替换 `_→̇_` 两边的公式, 再用 `_→̇_` 连接
...
**<u>定义</u>** 绑定变量的实例化 `φ [ t ∷]` 是指把公式 `φ` 中的 `# 0` 替换为项 `t`, 其他变量保持不变.
...
这两种概念仅仅是实用上的区别, 而没有本质不同. 语境和理论中的公式又叫做**公理 (axiom)**.

全称量化的引入规则中需要用到语境提升的概念, 而这又需要公式的提升, 它们都只是项的提升的简单推广.

**<u>定义</u>** 公式 `φ` 的提升 `↑ᵩ φ` 是指将 `φ` 中涉及的所有变量 (自然数) 替换为其后继.
...
有了以上准备, 我们终于可以定义什么叫证明. 我们采用所谓[**自然演绎 (natural deduction)**](https://zh.m.wikipedia.org/zh/%E8%87%AA%E7%84%B6%E6%BC%94%E7%BB%8E)的方案.
...
- 爆炸规则 `FalseE` : 如果有 `Γ` 下对恒假的证明树, 那么有 `Γ` 下对任意 `φ` 的证明树
- 皮尔士公理 `Peirce` : 无条件地, 有 `Γ ⊢ ((φ →̇ ψ) →̇ φ) →̇ φ`
...
**<u>定义</u>** 理论 `𝒯` 下对公式 `φ` 的证明树, 记作 `𝒯 ⊢ φ`, 是一个二元组
