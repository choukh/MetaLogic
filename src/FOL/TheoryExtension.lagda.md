...
给定 `𝒜 (suc n) ⊩ ⊥̇`, 由 `𝒜` 的定义和 `ImpIᵀ` 规则有 `𝒜 n ⊩ ¬̇ Ax n`, 也就是说有满足 `Γ ᴸ⊆ᴾ 𝒜 n` 的 `Γ` 满足 `Γ ⊢ ¬̇ Ax n`. 现在要证 `𝒜 n ⊩ ⊥̇`, 我们宣称 `Γ` 就是所需的语境, 也就是证 `Γ ⊢ ⊥̇`.
...
**<u>证明</u>** 给定 `𝒯 ⊩ Ψ n [ # n ]₀`, 我们有 `Γ ᴸ⊆ᴾ 𝒯` 满足 `Γ ⊢ Ψ n [ # n ]₀`. 现在要证 `𝒯 ⊩ ∀̇ Ψ n`, 我们宣称 `Ax n ∷ Γ` 就是要找的语境. 为此需要证明:
...
```agda
  ...
  ⨭ᶜ-Con {𝒯} {φ} {ψ} (Γ , Γ⊆ , Γ⊢) = 𝟙.map aux (⨭ᶜ-sub Γ 𝒯 φ Γ⊆) where
    aux : (Γ ᴸ⊆ᴾ 𝒯) ⊎ (Con 𝒯 ⨭ φ to 𝒯) → (𝒯 ⊩ ψ) ⊎ (Con 𝒯 ⨭ φ to 𝒯)
    aux = map₁ λ Γ⊆ → Γ , Γ⊆ , Γ⊢
```
