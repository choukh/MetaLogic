...
**<u>引理</u>** `𝒞ω` 具有蕴含分配性.  
**<u>证明</u>** 由 `C⊢-∈↔⊢`, 只需证 `φ →̇ ψ ∈ 𝒞ω` 当且仅当 `𝒞ω ⊢ᵀ φ → 𝒞ω ⊢ᵀ ψ`.
- 左边到右边, 由 `Ctxᵀ` 和 `ImpEᵀ` 即得.
- 右边到左边, 有前提 `𝒞ω ⊢ᵀ φ → 𝒞ω ⊢ᵀ ψ`, 要证 `φ →̇ ψ ∈ 𝒞ω`.
  由 `𝒞ω-isMaxCon`, 只需证 `𝒞ω ⨭ φ →̇ ψ ⊢ᵀ ⊥̇` 蕴含 `𝒞ω ⊢ᵀ ⊥̇`.
  由 `ImpEᵀ`, 我们分别证 `𝒞ω ⊢ᵀ ¬̇ (φ →̇ ψ)` 和 `𝒞ω ⊢ᵀ φ →̇ ψ` 即可.
  - 对于前者, 由前提 `𝒞ω ⨭ φ →̇ ψ ⊢ᵀ ⊥̇` 即得.
  - 对于后者, 由前者可得 `𝒞ω ⊢ᵀ φ`. 又由前提, 这意味着 `𝒞ω ⊢ᵀ ψ`, 所以 `𝒞ω ⊢ᵀ φ →̇ ψ`. ∎
...
- 右边到左边, 由 `𝒞ω` 对证明的封闭性, 只需证 `𝒞ω ⊢ᵀ ∀̇ φ`. 由极大全称化, 只需证 `∀ t → φ [ t ]₀ ∈ 𝒞ω`, 此即前提. ∎
